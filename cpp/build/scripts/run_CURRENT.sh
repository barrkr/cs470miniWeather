...
mkdir -p timings/CURRENT outputs/CURRENT

for p in 1 2 4 8 16 32 64; do
    echo "Running with $p workers"
    salloc -Q -n $p --gres=gpu mpirun ./parallelfor | grep CPU > timings/CURRENT/CURRENT_timing_$p.txt
    mv output.nc outputs/CURRENT/output_CURRENT_$p.nc
done
echo "Done"